...
Set at install time. These override the placeholder identities used in the examples below.

- **Agent identity:** `ai:claude-code`. Use `by: "ai:claude-code"` on every write, and `X-Agent-Id: ai:claude-code` for presence. Wherever the examples below show `ai:codex` or `ai:your-agent`, substitute `ai:claude-code`.
- **Open-in-Proof mode:** `only-when-asked`. Do NOT auto-create Proof documents for new markdown. Only create or open a Proof doc when the user explicitly asks for it (e.g. "put this in Proof", "share this as a Proof doc", "open this in Proof"). Otherwise keep markdown local.
- **Presence rule:** Bearer/share tokens authenticate access only; they do not identify presence. Always send a unique `X-Agent-Id` (here, `ai:claude-code`) on `/state` and `/presence` when presence should appear.
- **Bug reports:** If Proof returns a confusing error, stale read, or failed write, POST a report to `https://www.proofeditor.ai/api/bridge/report_bug` with a short summary, context, and raw evidence (request/response pairs, `x-request-id` values).

## Core Rules

- Include `by` on every write. Use `ai:<agent-name>` (this agent: `ai:claude-code`).
...
```json
{"type":"comment.add","by":"ai:claude-code","quote":"anchor text","text":"Comment body"}
{"type":"suggestion.add","by":"ai:claude-code","kind":"replace","quote":"old text","content":"new text"}
{"type":"suggestion.add","by":"ai:claude-code","kind":"replace","quote":"old text","content":"new text","status":"accepted"}
{"type":"rewrite.apply","by":"ai:claude-code","content":"# Rewritten markdown"}
```
...
curl -sS -X POST "http://localhost:4000/documents/<slug>/presence" \
  ...
  -H "Authorization: Bearer <token>" \
  -H "X-Agent-Id: ai:claude-code" \
  -d '{"agentId":"ai:claude-code","status":"thinking","summary":"Reviewing section 2"}'
```
...
- Hosted setup page: `https://www.proofeditor.ai/agent-setup`
- Reference SDK source: `https://github.com/EveryInc/proof-sdk`
- In-repo: `AGENT_CONTRACT.md`, `docs/agent-docs.md`, `docs/proof.SKILL.md`
